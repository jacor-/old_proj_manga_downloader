...
base_url = "http://d173.diamond.fastwebserver.de/series/Gantz/"
base_chapter = ""
base_image = "-"
base_extension = ".png"
...
def downloadNarutoFromTo(first,last):
    ...
    while book < last:
        ...
        chapter = 1
        while True:
            print "printing " + "wget("+get_url([fill_number(book,4),fill_number(chapter,3)])+")"
            system("wget "+get_url([fill_number(book,4),fill_number(chapter,3)])+" 2> error")
            if error() == True:
                break
            chapter = chapter + 1
