...
base_url = "http://bimg.anymanga.com/manga/doraemon/"
base_chapter = ""
base_image = "/001/"
base_extension = ".png"
...
def downloadNarutoFromTo(first,last):
    ...
    while book < last+1:
        ...
        chapter = 1
        while True:
            print "printing " + "wget("+get_url([fill_number(book,3),fill_number(chapter,3)])+")"
            system("wget "+get_url([fill_number(book,3),fill_number(chapter,3)])+" 2> error")
            if error() == True:
                break
            chapter = chapter + 1
